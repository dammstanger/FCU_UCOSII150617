...
/*********************************IIC interface adapt to I2C_1 Driver*******************************************/
int8_t i2cwrite(uint8_t addr, uint8_t reg, uint8_t len, uint8_t * data)
{
	if(IIC1_MultWrite(addr,data,reg,len))
	{
		return 1; //正确则返回0
	}
	else
...
{
	if(IIC1_MultRead(addr,buf,reg,len))
	{
		return 1;
	}
	else
...
{	u8 Add=0x55;
	...
	IIC1_MultRead(st.hw->addr,&Add,st.reg->who_am_i,1);
//	i2c_read(st.hw->addr,st.reg->who_am_i,1,&Add);
	printf("ID=%x\r\n",Add);
    /* Reset device. */
    data[0] = 0x80;//BIT_RESET;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(data[0])))
	{
		printf("i2c_write pwr_mgmt_1 error\r\n");
        return -1;
	}
    delay_ms(100);
	...
	{
		printf("i2c_read accel_offs error\r\n");
        return -1;
	}
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (mpu_set_accel_fsr(4))
        return -1;
    if (mpu_set_lpf(42))				//低通滤波器
        return -1;
    if (mpu_set_sample_rate(500))		//提高采样率//50  when the DMP is turned on,mpu_set_sample_rate should not be used.(DataSheet)
        return -1;
...
int mpu_run_self_test(long *gyro, long *accel)
{
    ...
    if (st.chip_cfg.dmp_on) {
        mpu_set_dmp_state(0);	//close DMP before other option
        dmp_was_on = 1;
    } else
        dmp_was_on = 0;
